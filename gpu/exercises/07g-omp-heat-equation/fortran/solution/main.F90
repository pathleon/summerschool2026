...
program heat_solve
  ...
  type(field), target :: current, previous    ! Current and previus temperature fields
  ...
  real(kind=dp), pointer, contiguous :: currdata(:,:), prevdata(:,:), tempdata(:,:)
  ...
  if (parallelization % rank == 0) then
     ...
     write(*,'(A, I5)') 'Number of devices: ', num_devices
  ...
  !$omp target data map(tofrom: currdata, prevdata)
  do iter = 1, nsteps
